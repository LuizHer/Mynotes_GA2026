...
$\varphi(U_i\times V_j)$ is closed in $W_{ij}$. However, if $(x,y)\in U_i\times V_i$, then 
$\varphi(x,y)=[\dots,x_ky_l,\dots]$, where we assume $x_i=1,y_j=1$. Now, we observe that the image is of the form 
$[\dots,u_{kl},\dots]$, where $u_{kl}=x_ky_l$. Then we conclude that the image is the locus of the system of homogeneous 
polynomials $u_{kl}-u_{kj}u_{il}=0,\;\forall k,l$, which is a closed set. It remains to show that 
